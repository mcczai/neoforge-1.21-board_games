...
> 详细设计见同目录 `CARD_DESIGN.md`（设计原则+技能草案+交互规划），**54 张卡表见 `CARDS.md`**（平衡版 v3），测试细节见 `P1_TEST.md`（测试清单+B1-B20 风险）。
> 最后更新：**54 张卡包数据包完整落地**（index/data/中英语言/48×64 卡面贴图，占位美术；生成管线 `CARD_DESIGN.md` §10.4）；卡池平衡版 v3；牌组与战场卡面 quad 比例对齐 3:4（`DuelTableBlockEntityRenderer`）。
...
| 陷阱 trap | `secret_*`→秘密区（上限 `DuelConfig.trapZoneLimit`）；`anvil_N`→点选敌方召唤物即伤；`*_thorns`→占槽反伤 | `secret_mine/arrow/wither/sculk/tnt` + `anvil_N`（代码保留，卡池改用 secret_arrow）+ `cactus_thorns` |
...
- **P1-2 / P1-3 / P1-4 / P1-5**：全部未实测。阻塞原因**仅剩** ① 用户缺第二客户端——② 已解除：默认卡包现为**完整 54 张**（召唤 21/魔法 13/陷阱 10/装备 10），四类结算与全部 P1 内建技能路径均可实测（卡表 `CARDS.md`）
...
1. ~~**54 张卡包 JSON + 贴图**~~ **✅ 已完成**：54 张 `cards/index` + `cards/data` + 中英 `lang`（各 162 key）+ 54 张 48×64 卡面贴图，卡框 24 个单独产出（`art/frames/`）；生成管线见 `CARD_DESIGN.md` §10.4——P1 四类结算实测的阻塞已解除
...
1. **近期（立即）**：~~制作 54 张卡包 JSON~~ **✅ 已落地** → 用户双客户端实测 P1 全流程（按 `P1_TEST.md` 与 B1-B20 风险表）→ 修 bug
...
- **拖拽施法**：先记录，P2 实现
- **卡池**：54 张设计稿平衡版 v3（`CARDS.md`，名字活力化+平衡复查），标准对局 10 法力/30 血/50 回合，含快攻/陷阱/成长/控制四流派与解牌矩阵，待实测微调
- **卡面视觉（用户定稿，`CARD_DESIGN.md` §10）**：6 派系×4 稀有度框（派系材质：末地石/黑曜石/原木苔藓/深板岩/红石矿石/工作台熔炉）；**稀有度表现：不加外圈，下 1/3 数值区矿物色包边 + 四角矿物宝石（铁/金/钻石/下界合金阶梯）+ 用户手绘的底部中央宝石保留（两者并存）**；卡面=上 2/3 立绘+下 1/3 数值、零文字；卡名/描述走 tooltip（仅 2D 渲染）；多卡背玩家自选；空槽不渲染贴图
- **数值区（2025 追加定稿）**：三槽顺序 **攻击 → 血量 → 费用**；槽内只画**背景图标**（攻击=铁剑、血量=玩家血量条单颗心、费用=绿宝石），**数值由运行时另行叠加渲染**（装备/药水的实时攻血变化才能正确显示）；图标 9×9，画布不足可缩小
- **卡面贴图规格**：48×64（3:4，用户手绘框原生尺寸）；世界端牌组/战场 quad 已对齐 3:4（`DuelTableBlockEntityRenderer`：`DECK_H = 3.2F/16F`、`BOARD_H = 3.4667F/16F`）
...
| 卡牌设计 | `docs/CARD_DESIGN.md` | 设计原则/数值规范/技能系统草案（§9）、拖拽交互（§8）、卡面视觉规范（§10） |
| 卡牌列表 | `docs/CARDS.md` | 54 张卡表（平衡版 v3）+ 平衡验证/四流派 + 修订记录 |
