...
> 最后更新：卡表独立为 `CARDS.md`，卡池平衡版 v3（54 张：新增僵尸小崽子、名字活力化、平衡复查）。
...
- **P1-2 / P1-3 / P1-4 / P1-5**：全部未实测。阻塞原因：① 用户缺第二客户端 ② 默认卡包只有 1 张羊（trap/skill=0，只能测站场兜底路径），四类结算需要 54 张卡池数据（`CARDS.md`）
...
1. **54 张卡包 JSON + 贴图**（`CARDS.md` 卡表，平衡版 v3）——解锁四类结算实测的关键
...
1. **近期（可选）**：制作 54 张卡包 JSON（`CARDS.md` 卡表 → `cards/index`+`cards/data`+占位贴图）→ 用户双客户端实测 P1 全流程 → 按 B1-B20 修 bug
