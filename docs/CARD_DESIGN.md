...
3. 卡面贴图 54 张：**已产出**（卡包 `textures/<id>.png`，48×64、3:4，程序化占位美术，见 §10.4；替换真美术时保持同尺寸同名即可）
...
- **结构**：6 派系 × 4 稀有度 = 24 个框；命名 `frame_{tribe}_{rarity}.png`；**实测分辨率 48×64（3:4）**——以用户手绘样图原生尺寸为准（末地/下界/自然 12 框为实画，虚空/红石/史蒂夫三派系由脚本按同风格补全）；HUD 手牌尺寸 44×62 与之近似 1:1
...
- **稀有度表现（用户定稿：包边+四角宝石）**：**不加外圈**（避免抢派系框主角、避免与黑曜石双层边缘糊成一片），改用两处矿物信号：
  ...
  - **四角宝石（用户追加定稿）**：四角各嵌一颗 3×3 矿物宝石，落在框的内角；**底部中央宝石保留不变**（两者同时存在，不改动用户已画的底部宝石）
  - 矿物阶梯：普通=**铁** `#C8C8CC`、稀有=**金** `#EBC64A`、罕见=**钻石** `#5FDCDC`、传说=**下界合金** `#6A4A80`——MC 原版装备阶梯语义，零学习成本
  - 已实现：24 个框全部带四角宝石 + 底部中央宝石（`art/tools/build_frames.ps1` 生成，输出 `art/frames/`）
- 角饰建议六派系统一结构（末地尖塔/下界火焰/自然枝叶/虚空幽匿/红石矿石/史蒂夫铁砧）——留待框 v2 迭代

### 10.2 卡面布局

- **上 2/3 = 立绘**（像素几何：卡面 48×64，框内洞 (6,5)-(43,55)，立绘区 y 5-38，数值区 y 39-55）
- **数值区三槽（用户定稿顺序：攻击 → 血量 → 费用）**：槽内只画**背景图标**，具体数值由运行时另行叠加渲染——**不烙进贴图**，这样装备/药水带来的实时攻血变化才会正确显示：
  - 攻击 = **铁剑**贴图；血量 = **玩家血量条单颗心**贴图；费用 = **绿宝石**贴图
  - 图标按 9×9 绘制（画布过小时允许缩小）；槽位坐标见 `art/frames/card_face_slots.json`
- 数值区边缘按稀有度**矿物色包边**（1px 外框 + 1px 内亮线）
...
### 10.4 卡面生成管线（占位美术，可一键重生成）

- **单一数据源**：`art/tools/cards.json`——54 张卡的全部卡池数据（id / 中英名 / 类型 / 派系 / 稀有度 / 费攻血 / skill / 立绘精灵 / 主色 accent / 中英技能描述 / MC 依据）+ 派系与稀有度元数据（矿物阶梯色）
- **`art/tools/build_frames.ps1`**：从 `art/reference/all.png`（用户手绘 12 框）取框 → 程序化补全虚空/红石/史蒂夫三派系 → 全部补四角宝石 → `art/frames/frame_{tribe}_{rarity}.png`（24 个）
- **`art/tools/build_faces.ps1`**：立绘（23 种程序化像素精灵，按卡 accent 着色 + 深色底自动提亮保证对比）× 数值区（矿物包边 + 攻/血/费三槽）× 叠框 → 卡包 `textures/<id>.png`（54 张）；同时输出 `art/frames/card_face_slots.json`（槽位像素坐标，供 P2 数值叠加对齐）
- **`art/tools/build_pack_json.ps1`**：生成 `cards/index/<id>.json` + `cards/data/<id>_data.json` + `lang/zh_cn.json` + `lang/en_us.json`（每卡 3 个 key：name/description/tooltip，232 行合计 162 key/语言）
- **替换真美术时**：直接覆盖 `textures/<id>.png`（保持 48×64、3:4）即可；或替换 `art/reference/all.png`（手绘框原图）后重跑脚本。派系材质补齐色：虚空=`#3A4048`+幽匿 `#1FA9A0`、红石=`#4A4A50`+红石 `#C02020`、史蒂夫=`#8B6239`+铁钉 `#C8C8CC`
